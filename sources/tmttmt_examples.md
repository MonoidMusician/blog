...
{# True or False #}
%type(Bool)("T" | "F") (@deriving Eq Ord)
{# The result of comparing within a total order #}
%type(ComparedTotal)("LT" | "EQ" | "GT") (@deriving Eq Ord)
{# A comparison to perform within a total order #}
%type(ComparisonTotal)("LT" | "LE" | "EQ" | "GE" | "GT") (@deriving Eq Ord)
...
shift: delta name2shift min
  ...
  shift delta name2shift min type#0 => type#1
  {# Increment `min` now that a new `name2shift` will be bound in `body##` #}
  shift delta name2shift [min] body#0 => body#1
shift: delta name2shift min
...
shift: delta name2shift min#0
  ["Forall" name2bind type#0 body#0]
  => ["Forall" name2bind type#1 body#1]:

  shift delta name2shift min#0 type#0 => type#1
  ...
  name2bind
    ...
    ? _:
      min#0 => min#1
    !
  shift delta name2shift min#1 body#0 => body#1
...
shift: delta name2shift min#0
  ...
  => ["Let" name2bind opt_type#1 value#1 body#1]:
    ...
    shift delta name2bind min#0 value#0 => value#1
    incr_if_eq min#0 name2shift name2bind => min#1
    shift delta name2bind min#1 body#0 => body#1
...
#}
shift: delta name2shift min other#0 => other#1:
  {# Unwrap `Expr` into `ExprF Expr` #}
  unlayer other#0 => layer#0 (: ExprF Expr :)
  {# Map over the structure and recurse #}
  map (\ child => shift delta name2bind min child) layer#0 => layer#1
  {# Wrap it back up #}
...
subst:: Name -> Nat -> Expr -> Expr -> Expr
subst: name2sub idx value ["Variable" name2sub idx] => value;
subst: name2sub idx value#0 expr#0 => expr#1:
  unlayer expr#0 => layer#1
  map_with_binder (|
    | [name2sub] child#0 => child#1:
      shift pos1(# +1 #) name2sub [] value#0 => value#1
      subst name2sub [idx] value#1 child#0 => child#1
    | _ child#0 => child#1:
      subst name2sub idx value#0 child#0 => child#1
  |) layer#0 => layer#1
  relayer layer#1 => expr#1

{# Helper for substitution with the correct shifts #}
shift_subst_shift: name2sub idx value#0 expr## => expr##:
  {# Make sure `["Variable" name2sub idx]` does not occur in `value##`
    by shifting everything up that is not below it #}
  shift pos1(# +1 #) name2sub idx value#0 => value#1
  ...
  shift neg1(# -1 #) name2sub idx expr## => expr##
