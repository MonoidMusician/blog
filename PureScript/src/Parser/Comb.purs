...
import Parser.Algorithms (addEOF'', fromSeed, generate, getResultC, getResultCM, indexStates, statesNumberedBy, toTable)
...
-- |
-- | Invariant:
-- | - length r.rule == r.resultant.length
...
-- |
-- | Invariants:
-- | - length is < r.length => (r.result `elem` [Partial, Failed])
-- | - length is == r.length => r.result /= Partial
-- | - r.result is == r.result (take r.length is)
...
parse name parser = do
  ...
  let Comb { grammar, rules } = named name (const parser)
  let states = statesNumberedBy identity grammar name
  let table = toTable states (indexStates states)
  -- Then we run it on an input
  addEOF'' >>> \input -> do
    stack <- Proto.parse table 0 input
...
named :: forall nt tok a. nt -> (Comb nt tok a -> Comb nt tok a) -> Comb nt tok a
named name rec =
  let
    recursive = rec $ Comb
      { grammar: mempty
  ...
  in Comb produced { grammar = produced.grammar <> newRules }

matchRule :: forall nt tok a. Array (Resultant (CCST nt tok) a) -> CCST nt tok -> Maybe a
