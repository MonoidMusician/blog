...
function RawBlock(h)
  if h.format == 'html' then
    if re.find(h.text, "'<'[Cc]*[Aa][Nn][Vv][Aa][Ss]") ~= nil then
      ...
      local fname = 'cache/render-canvas-' .. pandoc.sha1(h.text)
      local output = io.open(fname .. '.html')
      if output == nil then
        os.execute('mkdir -p ./assets/rendered')
        local t = pandoc.pipe('node', {pandoc.path.directory(PANDOC_SCRIPT_FILE) .. '/canvas.js', '--', './assets/rendered'}, h.text)
