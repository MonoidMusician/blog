...
function Math(h)
  ...
  local fname = 'cache/render-katex-' .. pandoc.sha1(h.text .. h.mathtype .. io.open('assets/katex_macros.tex'):read("*all"))
  ...
  if output == nil then
    ...
    input:write(h.text)
    input:close()
    os.execute('./cache/katex --no-throw-on-error --trust --macro-file assets/katex_macros.tex ' .. display_flag .. ' -i ' .. fname .. '.tex' .. ' -o ' .. fname .. '.html')
