...
import { DOM, Input, MaybePromise, Network, repollable, Timer } from "./APIs.js";
...
export function touch() {
    Resource.withScope(Resource.mkSubscope(Resource.noScope), () => {
        ...
        const select1 = (q: string) => document.querySelector(q);
        ...
        const touchPoints = (() => {
            ...
            return nice_colors.map(color => {
                const point: SVGCircleElement = template.parentElement!.appendChild(template.cloneNode()) as SVGCircleElement;
        ...
        let gestures = Stream.createRiver<Gesture.TouchPointGesture>();
        const { $text, $HTML } = DOM.VDOM;

        const dl = (items: Record<string, DOM.VDOM.Child[]>) => $HTML("dl", {}, Object.entries(items).map(([k, vs]) => {
            return [
                $HTML("dt", {}, k),
                vs.map(v => $HTML("dd", {}, v)),
            ];
        }));
        const $info = (f: (g: Gesture.TouchPointGesture) => string | number) => $text(gestures.stream.map(f));
